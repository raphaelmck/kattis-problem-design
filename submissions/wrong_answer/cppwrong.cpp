...
even though a valid ordering exists.

Thus, this approach may output IMPOSSIBLE or a suboptimal time when an optimal
solution exists.
*/
...
int main() {
    ...
    // Root the tree at the starting room
    distRoot.assign(n, 0);
    dfs_dist(S, -1);
    precompute_lca(S);

    // Wrong-answer greedy: handle alarms in increasing deadline order
    vector<int> order(K);
    ...
    for (int idx : order) {
        currTime += dist_tree(currPos, room[idx]);
        currTime += a[idx];
        if (currTime > d[idx]) {
            cout << "IMPOSSIBLE\n";
            return 0;
